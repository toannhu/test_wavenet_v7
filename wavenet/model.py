...
class WaveNetModel(object):
                ...
                for i, dilation in enumerate(self.dilations):
                    with tf.variable_scope('layer{}'.format(i)):
                        ...
                        if self.local_condition_channels is not None:
                            current['lc_filter'] = create_variable(
                                'lc_filter',
                                [1,
                                 self.local_condition_channels,
                                 self.dilation_channels])
                            current['lc_gate'] = create_variable(
                                'lc_gate',
                                [1,
                                 self.local_condition_channels,
                                 self.dilation_channels])
    ...
    def _generator_dilation_layer(self, input_batch, state_batch, layer_index,
        ...
        if local_condition_batch is not None:
            ...
            output_gate += tf.matmul(local_condition_batch, weights_lc_gate)
    ...
    def _create_generator(self, input_batch, global_condition_batch, local_condition_batch):
        ...
        outputs = []
        current_layer = input_batch
        lc_current_layer = local_condition_batch
            ...
            for layer_index, dilation in enumerate(self.dilations):
                with tf.name_scope('layer{}'.format(layer_index)):
                    ...
                    output, current_layer = self._generator_dilation_layer(
                        ...
                        global_condition_batch, lc_current_layer)
                    outputs.append(output)
    ...
    def predict_proba(self, waveform, global_condition=None, local_condition=None,
                      name='wavenet'):
